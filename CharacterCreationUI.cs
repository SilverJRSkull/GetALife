...
namespace GetALife.UI
{
    public class CharacterCreationUI : MonoBehaviour
    {
        ...
        // IMPORTANT:
        // - These are INTERNAL IDs used by CharacterGenerator ("Male"/"Female")
        // - The dropdown text is just a DISPLAY label (localized)
        private readonly List<string> genderIds = new List<string> { "Male", "Female" };
        private readonly List<string> genderDisplayLabels = new List<string>();

        private readonly List<string> monthOptions = new List<string>(12);
        ...
        {
            // Rebuild month + gender labels immediately when language changes.
            RebuildLocalizedDropdowns();
        }
        ...
        public void OnStartLifeClicked()
        {
            ...
            CharacterData character = new CharacterData
            {
                ...
                // Use INTERNAL gender ID, not visible label
                Gender = GetSelectedGenderId(),
        ...
        public void OnGenderChanged()
        {
            ...
            string genderId = GetSelectedGenderId();
            string newName = characterGenerator.GetRandomFirstName(genderId);
            ...
            {
                string[] namePool = characterGenerator.GetFirstNamesForGender(genderId);
        ...
        private void EnsureDropdowns()
        {
            ...
            // Rebuild gender + month dropdowns (localized labels)
            RebuildLocalizedDropdowns();
        ...
        private void RebuildLocalizedDropdowns()
        {
            RebuildGenderDropdown();
            RebuildMonthDropdown();
        }

        private void RebuildGenderDropdown()
        {
            if (genderDropdown == null)
                return;

            // Keep current selection by internal ID
            string currentId = GetSelectedGenderIdSafeFallback();
            int newIndex = Mathf.Max(0, genderIds.IndexOf(currentId));

            genderDisplayLabels.Clear();
            ...
            {
                genderDisplayLabels.Add("Homme");
                genderDisplayLabels.Add("Femme");
            }
            else if (code.StartsWith("es", StringComparison.OrdinalIgnoreCase))
            {
                genderDisplayLabels.Add("Hombre");
                genderDisplayLabels.Add("Mujer");
            }
            else
            {
                // English (default)
                genderDisplayLabels.Add("Male");
                genderDisplayLabels.Add("Female");
            }

            genderDropdown.ClearOptions();
            genderDropdown.AddOptions(genderDisplayLabels);
            genderDropdown.SetValueWithoutNotify(newIndex);
            genderDropdown.RefreshShownValue();
        }

        private void RebuildMonthDropdown()
        {
            if (birthMonthDropdown == null)
                return;

            int previousMonth = GetSelectedMonth(); // 1..12 or 0

            monthOptions.Clear();

            // Convert locale -> culture (best-effort)
            CultureInfo culture = ResolveCultureFromSelectedLocale();

            for (int month = 1; month <= 12; month++)
            {
                // GetMonthName already localized by culture
                string name = culture.DateTimeFormat.GetMonthName(month);

                // Force capital first letter (your request for fr/es)
                name = CapitalizeFirstLetter(name);

                monthOptions.Add(name);
            }
            ...
            {
                birthMonthDropdown.SetValueWithoutNotify(previousMonth - 1);
            }
            else
            {
                birthMonthDropdown.SetValueWithoutNotify(0);
            }

            birthMonthDropdown.RefreshShownValue();
        }

        private CultureInfo ResolveCultureFromSelectedLocale()
        {
            string code = LocalizationSettings.SelectedLocale?.Identifier.Code ?? "en";

            // Normalize common cases
            // Unity locale codes are typically "en", "fr", "es", etc.
            try
            {
                // Prefer explicit cultures if you want e.g. fr-FR / es-ES
                ...
                // Fallback: try use whatever code is
        ...
        private void RandomizeFromGenerator()
        {
            ...
            // Ensure dropdowns have correct labels for current locale
            RebuildLocalizedDropdowns();
            ...
            if (firstNameInput != null)
                firstNameInput.text = characterGenerator.GetRandomFirstName(genderId);
        ...
        // INTERNAL gender ID getter (Male/Female)
        private string GetSelectedGenderId()
        {
            if (genderDropdown == null)
                return "Male";

            int index = Mathf.Clamp(genderDropdown.value, 0, genderIds.Count - 1);
            return genderIds[index];
        }

        // Uses dropdown value but safe even if dropdown not built yet
        private string GetSelectedGenderIdSafeFallback()
        {
            if (genderDropdown == null)
                return "Male";

            int idx = genderDropdown.value;
            if (idx < 0 || idx >= genderIds.Count)
                return "Male";

            return genderIds[idx];
        }
